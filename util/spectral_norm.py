...
class LinearNormalGamma(torch.nn.Linear):

    def __init__(self, *args, spectral_norm_pi=1, **kwargs):
        super(LinearNormalGamma, self).__init__(*args, **kwargs)
        self.spectral_norm_pi = spectral_norm_pi
        if spectral_norm_pi > 0:
            self.register_buffer("u", torch.randn((1, self.out_features), requires_grad=False))
        else:
    ...
    def evidence(self, x):
        # return tf.exp(x) torch.nn.functional.softplus(x)
    ...
    def forward(self, input):
        ...
        output=F.linear(input, w_bar, self.bias)
        mu, logv, logalpha, logbeta = torch.split(output, output.shape[-1]//4, dim=-1) 
        v = self.evidence(logv)
        alpha = self.evidence(logalpha) + 1
        beta = self.evidence(logbeta)
        return torch.cat([mu, v, alpha, beta],dim=-1)
       
class LinearBeta(torch.nn.Linear):

    def __init__(self, *args, spectral_norm_pi=1, **kwargs):
        super(LinearBeta, self).__init__(*args, **kwargs)
        self.spectral_norm_pi = spectral_norm_pi
        if spectral_norm_pi > 0:
            self.register_buffer("u", torch.randn((1, self.out_features), requires_grad=False))
        else:
    ...
    def forward(self, input):
        ...
        output=F.linear(input, w_bar, self.bias)
        logalpha, logbeta = torch.split(output, output.shape[-1]//2, dim=-1) 
        alpha = self.evidence(logalpha) + 1e-6
